...
def get_products():
    ...
    result = [{'code':product.code, 'product': product.product, 'price': product.price, 'amount': product.amount, 'dates': product.dates} for product in products]
...
def get_product(product_code):
    ...
    return jsonify({'code':product.code, 'product': product.product, 'price': product.price, 'amount': product.amount, 'dates': product.dates})

@product_controller.route('/api/products', methods=['POST'])
def create_product():
    print("creando producto")
    data = request.json
    new_product = Products(product=data['product'], price=data['price'], amount=data['amount'], dates=data['dates'])
